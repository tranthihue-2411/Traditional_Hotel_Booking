    ...
    <form action="{{ route('admin.bookings.index') }}" method="GET">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
            ...
            <div>
                ...
                <select name="status"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 bg-white">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Từ ngày</label>
                <input type="date" name="date_from" value="{{ request('date_from') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Đến ngày</label>
        ...
        <div class="flex gap-2">
            <button type="submit"
                class="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2 rounded-lg text-sm font-semibold">
                Tìm kiếm
            </button>
            <a href="{{ route('admin.bookings.index') }}"
                class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-5 py-2 rounded-lg text-sm font-semibold">
                Xóa lọc
            </a>
        </div>
    </form>
        ...
        <table class="w-full min-w-max">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                    ...
                    <th class="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">Khách sạn</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        <p class="text-gray-600 text-sm max-w-[150px] truncate">{{ $booking->hotel->name }}</p>
                        <p class="text-gray-400 text-xs max-w-[150px] truncate">{{ $booking->room->name }}</p>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap {{ $s['class'] }}">
                            {{ $s['label'] }}
                        </span>
                    </td>
                    <td class="px-4 py-3">
                        @if($booking->is_paid)
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-green-100 text-green-700">
                            ✅ Đã thanh toán
                        </span>
                        @else
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-gray-100 text-gray-500">
                            ⏳ Chưa thanh toán
                        </span>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                        <a href="{{ route('admin.bookings.show', $booking) }}"
                            class="px-3 py-1 bg-teal-50 hover:bg-teal-100 text-teal-600 rounded text-xs font-semibold">
                            Xem
                        </a>
                    </td>
                </tr>
